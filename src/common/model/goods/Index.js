...
export default {
    ...
    return new Promise((resolve, reject) => {
      GoodsApi.detail({ goodsId })
        .then(result => {
          ...
          this.formData.goods = data.goodsInfo
          resolve()
        })
    })
  },
  ...
  getFieldsValue () {
    ...
    // 图片默认选中
    this.defaultList = goodsInfo.goods_images
    // 单规格数据
  ...
  getDeliveryList () {
    return new Promise((resolve, reject) => {
      DeliveryApi.all()
        .then(result => {
          this.formData.deliveryList = result.data.data.list
          resolve()
        })
    })
  },
  ...
  getServiceList () {
    return new Promise((resolve, reject) => {
      ServiceApi.all()
        .then(result => {
          const { data } = result.data
          this.formData.serviceList = data.list
          resolve()
        })
    })
  },
  ...
  getUserGradeList () {
    return new Promise((resolve, reject) => {
      GradeApi.all({ status: 1 })
        .then(result => {
          const { data } = result.data
          this.formData.userGradeList = data.list
          resolve()
        })
    })
  },
  ...
  setDefaultServiceIds () {
    ...
    if (!this.goodsId) {
      // 默认选中的id集
      const defaultServiceItems = serviceList && serviceList.filter(item => item.is_default)
      this.formData.defaultServiceIds = defaultServiceItems && defaultServiceItems.map(item => item.service_id)
    }
  },
