    ...
    <div id="edit-form-user">
        <el-dialog 
            ...
            <el-form 
                ...
                <el-form-item label="管理员姓名:" prop="real_name" :label-width="formLabelWidth" size="medium">
                    <el-input v-model="formData.real_name" autocomplete="off"></el-input>
                </el-form-item>

                <el-form-item label="用户名:" prop="user_name" :label-width="formLabelWidth" size="medium">
                    <el-input v-model="formData.user_name" autocomplete="off"></el-input>
                </el-form-item>

                <el-form-item v-if="!record.is_super" label="所属角色:" prop="roles" :label-width="formLabelWidth" size="medium">
                ...
                <el-form-item label="用户密码:" prop="password" label-width="100px" size="medium">
                    <el-input v-model="formData.password" type="password" autocomplete="off"></el-input>
                </el-form-item>

                <el-form-item label="确认密码:" prop="password_confirm" :label-width="formLabelWidth" size="medium">
                    <el-input v-model="formData.password_confirm" type="password" autocomplete="off"></el-input>
                </el-form-item>

                <el-form-item label="排序:" :label-width="formLabelWidth" size="medium">
                    <el-input-number
                    size="small" v-model="formData.sort" controls-position="right"
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    roles: [{ required: true, trigger: 'change', validator: validateSelect }],
                    password: [
        ...
        methods: {
            ...
            setFieldsValue() {
                console.log(this);
                const { record } = this;
                this.formData.roles = record.roleIds;
                this.formData.real_name = record.real_name;
                this.formData.user_name = record.user_name;
            ...
            submitForm(formName) {
                this.$refs[formName].validate(async (valid) => {
                    if (valid) {
                        const { data } = await UserApi.edit({ userId: this.record['store_user_id'], form: this.formData });
                        if(data.status == 200) {
                            // 添加成功关闭dialog并且重置表单
