      ...
      <el-form :model="form" ref="form" label-width="120px">
        ...
        <div class="tabs-content">
          ...
          <div class="tab-pane" v-show="tabKey == '0'">
            ...
            <el-form-item label="商品图片: " prop="imagesIds" size="small">
              <SelectImage multiple :maxNum="10" v-model="form.imagesIds" />
              <div class="form-item-help">
                <div class="extra">
            ...
            <el-form-item 
              ...
              prop="delivery_id" 
              size="small"
              :rules="[{ required: true, message: '请选择运费模板', trigger: 'change' }]"
            >
              <el-select
          ...
          <div class="tab-pane" v-show="tabKey == '3'">
            ...
            >
              ...
              />
              <div class="form-item-help">
                <div class="extra">一句话简述，例如：此款商品美观大方 性价比较高 不容错过</div>
              </div>
            </el-form-item>
            ...
            <el-form-item 
              label="积分赠送: " size="small"
              prop="is_points_gift"
              :rules="[{ required: true, message: '请选择积分赠送', trigger: 'blur' }]"
            >
            ...
            <el-form-item 
              label="积分抵扣: " size="small"
              prop="is_points_discount"
              :rules="[{ required: true, message: '请选择积分抵扣', trigger: 'blur' }]"
            >
              ...
              <div class="form-item-help">
                <div class="extra">开启后用户购买此商品可以使用积分进行抵扣</div>
              </div>
            </el-form-item>

            <el-divider content-position="left">会员折扣设置</el-divider>
            <el-form-item 
              label="会员折扣: " size="small"
              prop="is_enable_grade"
              :rules="[{ required: true, message: '请选择会员折扣', trigger: 'blur' }]"
            >
              <el-radio-group v-model="form.is_enable_grade" @change="onForceUpdate(true)">
                <el-radio :label="1">开启</el-radio>
                <el-radio :label="0">关闭</el-radio>
              </el-radio-group>
              <div class="form-item-help">
                <div class="extra">开启后会员折扣，会员购买此商品可以享受会员等级折扣价</div>
              </div>
            </el-form-item>

            <el-form-item 
              v-if="form.is_enable_grade"
              label="会员折扣设置: " size="small"
              prop="is_alone_grade"
              :rules="[{ required: true, message: '请选择会员折扣设置', trigger: 'blur' }]"
            >
              <el-radio-group v-model="form.is_alone_grade" @change="onForceUpdate(true)">
              ...
              <div v-if="form.is_alone_grade" style="margin-top: 18px">
                <el-form-item
                  v-for="item in formData.userGradeList" :key="item.grade_id"
                  size="small" :rules="[{ required: true, message: '折扣率不能为空', trigger: 'blur' }]"
                >
                  ...
                  <el-input-number 
                    ...
                    controls-position="right"
                    :min="0" :max="9.9"
                    :precision="1" style="width: 100px"
                  />
                  <span style="margin-left: 10px; color: rgba(0, 0, 0, 0.45)">(折)</span>
                </el-form-item>
              </div>
              <div class="form-item-help">
                <div class="extra">开启后会员折扣，会员购买此商品可以享受会员等级折扣价</div>
        ...
        <div class="tab-pane btn-submit">
          <el-form-item size="small" label-width="120px">
            <el-button type="primary" @click="handleSubmit" v-loading="isBtnLoading">提 交</el-button>
          </el-form-item>
        </div>
      </el-form>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    async onFormSubmit(values) {
      ...
      const { data } = await GoodsApi.add({ form: values })
      this.$notify({
