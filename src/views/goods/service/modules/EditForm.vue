    ...
    export default {
        ...
        methods: {
            ...
            setFieldsValue() {
                const { record } = this;
                this.$nextTick(() => {
                    // pick()方法将一个对象中的某些属性抽离到一个新的对象中
                    const newData = pick(record, ['name', 'summary', 'is_default', 'status', 'sort']);
            ...
            submitForm(formName) {
                this.$refs[formName].validate(async (valid) => {
                    if (valid) {
                        const { data } = await Api.edit({ serviceId: this.record['service_id'], form: this.formData })
                        this.visible = false;
                        this.$notify({
